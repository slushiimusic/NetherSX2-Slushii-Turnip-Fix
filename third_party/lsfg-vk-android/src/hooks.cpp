...
namespace {
    ...
    VkResult myvkCreateSwapchainKHR(
        ...
        try {
            ...
            // LsContext owns the async worker thread and is non-movable, so it
            // must be constructed in place; emplace() would need a move ctor.
            swapchains.try_emplace(*pSwapchain,
