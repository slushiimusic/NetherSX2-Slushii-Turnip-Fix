...
namespace Layer {
    ...
    VkResult ovkQueueSubmit(
        ...
        VkFence fence);
    /// Block until the host Vulkan device has finished all queue work.
    VkResult ovkDeviceWaitIdle(VkDevice device);
    ...
    /// Optional external VkQueue lock (shim hooks QueueSubmit + LSFG worker).
    void ovkSetQueueLock(void (*lock)(void), void (*unlock)(void));
