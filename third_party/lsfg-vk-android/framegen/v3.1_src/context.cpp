...
void Context::present(Vulkan& vk,
    ...
    if (historyOnly) {
        // Advance the three-capture feature history and input parity, but do
        // not synthesize images the Android artifact guard will discard.
        // Input ownership still returns to the consumer after this submission.
#ifdef __ANDROID__
...
#endif
        data.cmdBuffer1.end();
        auto& fence = data.completionFences.at(0);
        fence = Core::Fence(vk.device);
        data.cmdBuffer1.submit(vk.device.getComputeQueue(), fence,
            waits, std::nullopt, {}, std::nullopt);
        data.pendingFenceCount = 1;
        ++this->frameIdx;
        return;
    }
    data.cmdBuffer1.end();
