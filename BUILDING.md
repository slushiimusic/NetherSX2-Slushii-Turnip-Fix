...
On macOS, install the Android SDK (platform35 and build-tools35.0.0), NDK26.1.10909125, Java11, Python3, ripgrep and glslangValidator. Put apksigner.jar next to turnip-shim/repack-apk.py. Supply a Pink base APK containing the emulator and the LSFG library; FRAMEGEN reuses that library unless turnip-shim/lsfg-lib contains a replacement. Native LSFG modifications and their upstream revisions are included under third_party and THIRD_PARTY.json. The v26500720 motion fix is in liblsfg-android.so: use the v26500720 FRAMEGEN APK as the base, or rebuild that library from the modified sources with their upstream dependencies and place it in turnip-shim/lsfg-lib. Repacking an older base without the updated library retains the older artifact guard.

```bash
VARIANT=FRAMEGEN VERSION_CODE=26500720 bash build-nova-thor.sh /path/to/Pink-base.apk
VARIANT=NONFRAMEGEN VERSION_CODE=26500720 bash build-nova-thor.sh /path/to/Pink-base.apk
